...
class AuthService {
  ...
  async cadastrarUsuario(data) {
    try {
      ...
      const emailJaExiste = await Usuario.pegarPeloEmail(data.email);
      if (emailJaExiste) {
        throw new Error('Email já cadastrado!');
      }

      data.senha = await bcryptjs.hash(data.senha, 8);  // Criptografa a senha recebida antes de guardar no db.
