...
class LivrosImagensService {
  ...
  async cadastrarImagem(req) {
    try {
      if (!req.body.livroId) {
        throw new Error('É obrigatório informar o ID do livro vinculado à imagem.');
      }

      if (!constants.imageMimeType.includes(req.file.mimetype)) {
        throw new Error(`O formato ${req.file.mimetype} não é suportado.`);
      }

      if (req.file.size > 5000) {
        throw new Error('O limite máximo é de 5000kb');
      }
